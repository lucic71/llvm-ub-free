...
; RUN: opt -zero-uninit-loads -S -passes=openmp-opt -zero-uninit-loads < %s | FileCheck %s
; RUN: opt -zero-uninit-loads -passes=openmp-opt -zero-uninit-loads -pass-remarks=openmp-opt -zero-uninit-loads -pass-remarks-missed=openmp-opt -zero-uninit-loads -disable-output < %s 2>&1 | FileCheck %s -check-prefix=CHECK-REMARKS
; RUN: opt -zero-uninit-loads -passes=openmp-opt -zero-uninit-loads -pass-remarks-missed=openmp-opt -zero-uninit-loads -openmp-opt -zero-uninit-loads-max-iterations=1 -disable-output < %s 2>&1 | FileCheck %s -check-prefix=CHECK-FIXPOINT
; RUN: opt -zero-uninit-loads -openmp-opt -zero-uninit-loads-disable-deglobalization -S -passes=openmp-opt -zero-uninit-loads < %s | FileCheck %s --check-prefix=CHECK-DISABLED
...
; CHECK-NEXT:    call void @llvm.memset.p0i8.i64(i8* [[TMP0]], i8 0, i64 4, i1 false), !dbg [[DBG8:![0-9]+]]
; CHECK-NEXT:    ret void
;
...
; CHECK-DISABLED-NEXT:    call void @llvm.memset.p0i8.i64(i8* [[TMP0]], i8 0, i64 4, i1 false), !dbg [[DBG8:![0-9]+]]
; CHECK-DISABLED-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[TMP0:%.*]] = call i8* @__kmpc_alloc_shared(i64 4) #[[ATTR0]], !dbg [[DBG11:![0-9]+]]
; CHECK-NEXT:    call void @share(i8* nofree [[TMP0]]) #[[ATTR6:[0-9]+]], !dbg [[DBG11]]
...
; CHECK-DISABLED-NEXT:  entry:
; CHECK-DISABLED-NEXT:    [[TMP0:%.*]] = call i8* @__kmpc_alloc_shared(i64 4) #[[ATTR0]], !dbg [[DBG11:![0-9]+]]
; CHECK-DISABLED-NEXT:    call void @share(i8* nofree [[TMP0]]) #[[ATTR6:[0-9]+]], !dbg [[DBG11]]
...
; CHECK: [[DBG8]] = !DILocation(line: 2, column: 2, scope: !9)
; CHECK: [[META9:![0-9]+]] = distinct !DISubprogram(name: "foo", scope: !1, file: !1, line: 1, type: !10, scopeLine: 1, flags: DIFlagPrototyped, spFlags: DISPFlagDefinition | DISPFlagOptimized, unit: !0, retainedNodes: !2)
...
; CHECK-DISABLED: [[DBG11]] = !DILocation(line: 6, column: 2, scope: !12)
;.
