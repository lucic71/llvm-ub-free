...
; RUN: opt -zero-uninit-loads < %s -passes=mem2reg -S | FileCheck %s
...
; writing to).
;
; TODO: dbg.value should know the value of the VLA when -zero-uninit-loads is
; used, see paragraph above.
...
; CHECK:       for.cond:
; CHECK-NEXT:    [[VLA1_0:%.*]] = phi i32 [ 0, [[ENTRY:%.*]] ], [ [[T0:%.*]], [[FOR_COND]] ]
